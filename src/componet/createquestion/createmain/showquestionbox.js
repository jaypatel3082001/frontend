...
function Showquestionbox({ showQuestion }) {
  ...
  const filteredData = useMemo(() => {
    return inputs.Tablemanuplation?.data?.filter(
      (info) =>
        inputs.Tablemanuplation.idstores === info._id && inputs.openpop === true
    );
  }, [
    inputs.Tablemanuplation.data,
    inputs.Tablemanuplation.idstores,
    inputs.openpop,
  ]);
        ...
        >
          <div className="bg-white w-3/4 md:w-1/2 xl:w-1/3 p-6 rounded-lg shadow-lg">
            <div className="flex justify-between">
              <div className="font-bold text-xl">Mark: {info.weightage}</div>
              <div
